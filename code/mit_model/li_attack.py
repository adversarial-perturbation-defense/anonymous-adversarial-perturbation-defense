...
##
## 2016, Nicholas Carlini <nicholas@carlini.com>.
##
...
FIX_TAU = False         # whether tau is fixed during the perturbation process.
INITIAL_TAU = 1.0       # initial value of tau
class CarliniLi:
  def __init__(self, sess, model,
               ...
               const_factor=CONST_FACTOR, fix_tau = FIX_TAU,
    ...
    Args:
      ...
      fix_tau: If true, we use only a fixed value of tau.

      initial_tau: The initial value of the constant tau defining the bound for
        the second loss
    """
    ...
    # Whether tau is fixed during the perturbation process.
    self.FIX_TAU = fix_tau
    # Initial value of tau
    self.INITIAL_TAU = initial_tau

    # Get a doit function for doing gradient descent.
    self.grad = self.gradient_descent(sess, model)

  def gradient_descent(self, sess, model):
    """Return a doit function for doing gradient descent.

    Args:
      sess: session.
      model: A CIFARModelShell defined in model_shells.py.

    Returns:
      a doit function for searching adversarial examples with gradient descent.
    """
    ...
    orig_output = model.symbolic_predict(tf.tanh(timg)/2)
    ...
    def doit(oimgs, labs, starts, tt, CONST, verbose=False):
      ...
      while CONST < self.LARGEST_CONST:
        if verbose:
          logging.info('try const %s', CONST)
        # Step 1: For a particular constant try a number of iterations.
        for step in range(self.MAX_ITERATIONS):
          ...
          if step % (self.MAX_ITERATIONS//10) == 0:
            sess_run_stats = sess.run(
              (loss, loss1, loss2), feed_dict=feed_dict)
            if verbose:
              logging.info('%s, %s', step, sess_run_stats)
        ...
        # Step 2: If we didn't succeed, increase constant and try again.
        CONST *= self.const_factor

    return doit

  def attack(self, imgs, targets, verbose=False):
    """
    ...
    Args:
      imgs: Features to attack.
      targets: Target of attack See above for how targets are constructed.
      verbose: Print a debugging info for debugging the attack.

    Returns:
      An numpy array of results.
    """
    r = []
    for img,target in zip(imgs, targets):
      r.extend(self.attack_single(img, target, verbose))
    return np.array(r)

  def attack_single(self, img, target, verbose=False):
    """
    Run the attack on a single image and label

    Args:
      image: A feature vector to attack.
      target: Target of attack. See above for how targets are determined.
      verbose: Print a debugging info for debugging the attack.
    ...
    logging.info('attack_single verbose=%s', verbose)

    # the previous image
    prev = np.copy(img).reshape(
        (1, self.model.image_size, self.model.image_size,
    ...
    if self.FIX_TAU:
      ...
      res = self.grad(
          [np.copy(img)], [target], np.copy(prev), tau, const, verbose)
      if res == None:
